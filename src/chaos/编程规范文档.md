...
| format    | 仅格式化代码                                     |
| bug       | 发现bug,可在本地提交声明bug编号 和 bug说明                |
...
| serious              | 重大更新                                |
| nightly              | 每日更新                                |
| sub\(<sub_name>,...) | 指定的子项目,例如 feat\[sub\(my_sub_crate)] |
...
##### \[milestone(\<version>)]
标记当前todo位于那一个阶段应该被完成
用法 `#todo[milestone(1.0.0)]`
`version` 表示一个阶段, 一般指的是 版本

##### \[priority(\<token>)]
描述当前todo的优先级
`token` 可以是 数字`[0,9]` ,或 `[low,mid,high]`
...
#### \#trace
用于追踪代码, 目的是( 强调上下文, debug追踪 )
`#trace[#trace_code,...]` 

##### \[trace_code]
`trace_code` 可以是任意名字, 但必须以`#`开头,且不能有空格
